...
function muslprti_handle_import_preview() {
    ...
    if (!isset($_FILES['import_file']['error']) || $_FILES['import_file']['error'] !== UPLOAD_ERR_OK) {
        ...
        $error_message = isset($upload_errors[$error]) ? $upload_errors[$error] : esc_html__('Unknown upload error', 'muslim-prayer-times');
        wp_send_json_error($error_message);
        return;
    }

    // Get file info - DON'T sanitize tmp_name!
    $file_tmp = $_FILES['import_file']['tmp_name'];
    $file_name = isset($_FILES['import_file']['name']) ? sanitize_file_name($_FILES['import_file']['name']) : '';

    // Check file type
    $file_type = wp_check_filetype(basename($file_name), array('csv' => 'text/csv'));
    if (!$file_type['ext']) {
    ...
    if (!is_uploaded_file($file_tmp)) {
        wp_send_json_error(esc_html__('Security check failed: Invalid file upload', 'muslim-prayer-times'));
        return;
    }
    ...
    if ($content === false) {
        wp_send_json_error(esc_html__('Failed to read uploaded file', 'muslim-prayer-times'));
        return;
    }
...
function muslprti_handle_import() {
    ...
    if (!isset($_FILES['import_file']['error']) || $_FILES['import_file']['error'] !== UPLOAD_ERR_OK) {
        $error = isset($_FILES['import_file']['error']) ? intval($_FILES['import_file']['error']) : UPLOAD_ERR_NO_FILE;
        wp_send_json_error(esc_html__('File upload error code: ', 'muslim-prayer-times') . esc_html($error));
        return;
    }
    ...
    // Get the date format from POST data
    $date_format = isset($_POST['date_format']) ? sanitize_text_field(wp_unslash($_POST['date_format'])) : 'Y-m-d';
            ...
            if (isset($row_data['day'])) {
                ...
                if ($date_obj) {
                    ...
                    if ($result !== false) {
                        ...
                        $error_count++;
                        $errors[] = sprintf('Failed to import date: %s', $row_data['day']);
                    }
                } else {
                    $error_count++;
                    $errors[] = sprintf('Invalid date format for: %s', $row_data['day']);
                }
            }
        }
    }
